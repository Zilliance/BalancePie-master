...
extension AddSliceViewController: UITableViewDataSource
{
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        case (.badFeelings?):
            let cell = tableView.dequeueReusableCell(withIdentifier: "valuesCell", for: indexPath) as! ActivityTableViewCell
            let text = self.newActivity.feeling == .good ? self.newActivity.neutralValues.map{$0.name}.joined(separator: "\n") : self.newActivity.badValues.map{$0.name}.joined(separator: "\n")
...
extension AddSliceViewController: UITableViewDelegate, UIViewControllerTransitioningDelegate
{
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        case .badFeelings?:
            ...
            let values = self.newActivity.feeling == .good ? Value.neutralValues : Value.badValues
            
            let initialIndexes = self.newActivity.feeling == .good ? values.flatMap({self.newActivity.neutralValues.index(of: $0) == nil ? nil : values.index(of: $0)}) : values.flatMap({self.newActivity.badValues.index(of: $0) == nil ? nil : values.index(of: $0)})
            let valueType: ValueType = self.newActivity.feeling == .good ? ValueType.neutral : ValueType.bad
